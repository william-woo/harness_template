...
올리면 더 오래된 미처리분이 **소실**되고, ② 전부 못 받으면 아무것도 처리하지 않으면
백로그가 상한을 넘을 때 **신규 메시지조차 막히는 기아**가 되며(실측 rc=0 으로 숨음),
③ 부분 처리만 하면 dedup 이 없어 **복제**된다. 셋을 동시에 막는 것은 "부분 처리 +
`slack-seen.json` dedup + cursor 는 완전 수집 회차에만 전진" 조합뿐이다.

**남는 한계 (정직하게)**: 백로그가 50페이지를 넘으면 가장 오래된 구간은 수집이 한 번
완주할 때까지 밀린다. 최신 창은 매 폴링 흐르므로 운영에는 지장이 없지만, 긴 이력의
채널에 합류할 때는 `CONSORTIUM_SLACK_OLDEST` 로 시작점을 옮기는 편이 낫다.
