...
③ 개발 (DEVELOP)   developer
     feature 1개씩 구현 + 단위 테스트 (한 번에 하나)
        ...
        ▼
④ 검증 (VERIFY)    reviewer → qa (+ design-review / qa-browser 조건부)
     reviewer: 코드 품질·보안·성능 (APPROVED → status: qa)
     qa: acceptance_criteria E2E 검증 → passes:true (QA 단독)
     design-review/qa-browser: UI 변경 시
        │  [PM 게이트] 성공지표(brief) 충족 확인됐나?
        ▼
