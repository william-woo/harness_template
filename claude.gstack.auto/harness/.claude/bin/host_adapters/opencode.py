...
# 렌더 산출물 마커 — 이 줄이 있는 파일만 stale 정리 대상이다.
#
# 왜 필요한가 (F023 리뷰 MUST-1): 예전 정리 로직은 출력 디렉토리의 **모든** `.md` 를
# "이번에 생성되지 않았으면" 지웠다. 실측으로 `--agents-out docs/adr` 하나에 ADR 18건
# 중 9건이, `--agents-out .` 에 `CLAUDE.md`·`README.md` 가 사라졌다.
# python 내부 `unlink` 라 `permissions.ask` 의 `rm` 패턴도 훅도 잡지 못한다 —
# CLAUDE.md 규칙 #1 이 경고한 "선언 규칙이 못 잡는 삭제 경로" 가 바로 이것이다.
_RENDER_MARKER = "<!-- generated by host.py render — do not edit; edits are overwritten -->"


def _is_rendered(path: Path) -> bool:
    """이 파일이 렌더러가 만든 것인지 (첫 2KB 에 마커가 있는지)."""
    try:
        return _RENDER_MARKER in path.read_text(encoding="utf-8", errors="replace")[:2048]
    except OSError:
        return False


def _prune_stale(out: Path, generated: set, kind: str) -> None:
    """이번에 생성되지 않은 **렌더 산출물만** 지운다.

    마커가 없는 파일은 사람이 둔 것이므로 삭제하지 않고 경고만 한다 —
    지우는 쪽이 아니라 남기는 쪽이 복구 가능한 선택이다.
    """
    kept = []
    for existing in sorted(out.glob("*.md")):
        if existing.name in generated:
            continue
        if _is_rendered(existing):
            existing.unlink()
        else:
            kept.append(existing.name)
    if kept:
        print(f"  [WARN] {kind} 출력 디렉토리에 렌더 산출물이 아닌 파일이 있습니다 — "
              f"건드리지 않았습니다: {', '.join(kept[:5])}"
              + (f" 외 {len(kept) - 5}건" if len(kept) > 5 else ""))
...
class OpenCodeAdapter(HostAdapter):
    ...
    def render_agents(
        ...
        for agent_file in sorted(src.glob("*.md")):
            ...
            out_file.write_text(_RENDER_MARKER + "\n" + result, encoding="utf-8")
            generated.append(agent_file.name)

        # stale 정리 — **렌더 산출물만** (마커 기준). 상세는 `_prune_stale` 참조.
        _prune_stale(out, set(generated), "agent")
    ...
    def render_commands(
        ...
        for cmd_file in sorted(src.glob("*.md")):
            ...
            if result is None:
                continue
            (out / cmd_file.name).write_text(_RENDER_MARKER + "\n" + result, encoding="utf-8")
            generated.append(cmd_file.name)

        _prune_stale(out, set(generated), "command")
