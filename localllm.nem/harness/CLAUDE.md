...
>
> 🧠 **이 변형은 `localllm.nem` — nemotron 전용 분기입니다** (F030 / ADR-021).
> 부모 `localllm` 의 qwen2.5 2 티어(14B 생성형 / 32B 판정) 대신 **`nemotron-3-nano:30b`
> 단일 모델**을 전 9 역할에 쓴다. nemotron 은 **추론 모델**이라 도구 호출 전에 추론 토큰을
> 먼저 쓰므로, 전역 설정에 **`limit.output` 을 반드시 함께 등재**해야 도구 호출이 성립한다
> (측정 11 결과 10). `opencode-setup.sh` 가 등재·보정을 자동 처리한다.
...
| 판정(judge) 역할 | reviewer/qa = fable | **전 역할 동일 모델** (`nemotron-3-nano:30b`) — 티어 없음 |
| 출력 예산 | 호스트가 관리 | **`limit.output` 전역 등재 필수** — 추론 모델이라 예산 부족 시 도구 호출 무산 |
...
# 단일 역할 직접 호출 (전 역할 동일 모델)
opencode run --agent developer --model ollama/nemotron-3-nano:30b "<요청>"
# 판정 역할도 같은 모델 (티어 없음 — ADR-021)
opencode run --agent reviewer --model ollama/nemotron-3-nano:30b "<요청>"
...
> 모델 무관 100% 신뢰. 이 변형은 전 역할이 같은 모델이므로 judge 승급 대신
> **추론 예산 확보**(`limit.output`)가 판정 품질의 전제 조건이다.
