---
description: "QA 브라우저 자동화"
---

# /qa-browser — QA 브라우저 자동화
...
```
/qa-browser                                  # downstream 기본 (현재 in-progress Feature 자동 선택)
/qa-browser --feature=F012                   # 특정 Feature ID 의 acceptance 자동 번역
/qa-browser --target=http://localhost:3000   # URL 직접 지정
/qa-browser --scope=self                     # 하네스 자체 정의 검증 (dry-run)
/qa-browser --rerun=last                     # 직전 실패한 스크립트 재실행
/qa-browser --rerun=BLOCK-QA-2               # 특정 BLOCK 재실행
```
...
QA 에이전트 (PASS 판정 권한 보유)
  └→ /qa-browser --feature=Fxxx (스크립트 생성 + 실행)
...
Playwright 가 설치되어 있지 않습니다. 아래 중 하나를 선택하세요:
  ...
  설치 후 재호출: /qa-browser --feature=F012
