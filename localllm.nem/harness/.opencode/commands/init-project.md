...
(선택) `/backup-sync init` 으로 백업 리포 설정 — 다른 머신·팀원과 산출물 동기화.
...
OLLAMA_HOST=http://172.16.10.217:11434 OLLAMA_MODEL=nemotron-3-nano:30b \
  bash .claude/bin/opencode-setup.sh
```

설정 후 하네스 활용:
```bash
opencode run --model ollama/nemotron-3-nano:30b "<요청>"
