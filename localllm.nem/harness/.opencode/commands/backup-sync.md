---
description: "다운스트림 백업 동기화"
---

# /backup-sync — 다운스트림 백업 동기화
...
```
/backup-sync               # 기본 동작 = sync
/backup-sync init          # 백업 리포·브랜치 초기 설정
/backup-sync status        # 마지막 sync 정보 + 변경 미리보기
/backup-sync config show   # 현재 설정 조회
/backup-sync self          # 셀프 dry-run — 의존성 체크
```
...
| 세션 | 범위 | 상태 |
|---|---|---|
| 세션 1 | `sync` 코어 + 보안 BLOCK + `/backup-sync` 커맨드 | 구현 완료 |
