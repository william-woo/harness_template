---
description: "이종 에이전트 오케스트레이션 (supervisor)"
---

# /orchestrate — 이종 에이전트 오케스트레이션 (supervisor)
...
```
/orchestrate "<복합 요청>"
/orchestrate --feature=F0XX
/orchestrate --steps=research,design,dev   # 단계 명시 override (선택)
```

예:
```
/orchestrate "사용자 인증에 OAuth2 PKCE 추가"
/orchestrate "결제 페이지 신규 디자인 + Stripe 연동"
/orchestrate --feature=F042
/orchestrate "Redis 캐싱 레이어 도입" --steps=research,architect,dev,reviewer
```
...
| 도구 | 범위 | 출력 | 후속 |
|---|---|---|---|
| `/plan-full` (F002) | **설계 체인** — 요구사항 → Feature 분해 → ADR 작성 → 설계 감사 | feature_list 추가 + ADR | Developer 가 별도 실행 |
| **`/orchestrate` (F013)** | **실행 라우팅** — 요구사항 → 리서치/디자인/코딩 분기 → 에이전트 순차 spawn → 산출물 통합 | 핸드오프 디렉토리 + 최종 통합 리포트 | QA + 사용자 검수 |
...
/design-review --scope=downstream
