...
**현재 설정** (nemotron 단일 모델 — ADR-021):

| 역할 | 모델 |
|---|---|
| 전 9 역할 (architect · designer · developer · gatekeeper · planner · product-manager · qa · researcher · reviewer) | `nemotron-3-nano:30b` |
| (기본값) | `nemotron-3-nano:30b` |

| 모델 | 크기 | 추론 모델 | 근거 |
|---|---|:-:|---|
| `nemotron-3-nano:30b` | 24.3GB | ✅ | 이 변형의 전 역할 모델. 능력 선언 `completion, tools, thinking` |
| `nemotron-mini:latest` | 2.7GB (4.2B) | ❌ | 경량 옵션 — provider 에 등재만 하고 **역할 배정 없음**. 미검증 |

> **왜 단일 모델인가**: 부모 `localllm` 은 생성형/판정 2 티어(14B/32B)였다. 이 변형은
> nemotron 계열의 성능을 **혼선 없이** 측정하는 것이 목적이므로 티어를 두지 않는다.
> `nemotron-mini` 를 gatekeeper 에 붙이면 4.2B 라는 미검증 변수가 판정에 섞인다.

### ⚠️ 추론 예산이 없으면 도구 호출이 무산된다 (측정 11 결과 10 — 이 변형의 존재 이유)

nemotron 은 **추론 모델**이다. 도구를 호출하기 **전에** 추론 토큰을 먼저 쓰고, 출력 예산이
그보다 작으면 추론 중간에서 잘려 **도구 호출 델타가 아예 생성되지 않는다** (`finish=length`).
모델이 무엇을 해야 하는지 알고 있어도 말할 지면이 없는 상태다.

따라서 전역 설정 등재 시 `limit` 을 반드시 함께 넣는다:

```jsonc
"nemotron-3-nano:30b": {
  "name": "...",
  "limit": { "context": 32768, "output": 4096 }   // ← output 없으면 도구 호출 실패
}
```

`opencode-setup.sh` 가 이를 자동 처리하고, **이미 `limit` 없이 등재된 항목도 보정**한다.
`limit` 은 `context` 와 `output` 을 **둘 다** 요구한다 — 하나만 넣으면 OpenCode 가
`Configuration is invalid` 로 **모든 실행을 즉시 거부**한다.
