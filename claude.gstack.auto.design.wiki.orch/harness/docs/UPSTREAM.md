---
name: gstack-upstream-auto
type: provenance
variant: claude.gstack.auto
parent_variant: claude.gstack
derived_at: 2026-05-27
autonomous_mode: true
autonomous_policy_rules: [no-prompt-within-workdir, agents-review-borderline, escalate-auth-and-external]
...
# Upstream Provenance — gstack (Autonomous Mode 변형)

이 파일은 `claude.gstack.auto` 변형의 출처와 정책을 기록한다.

## 변형 계보

```
gstack (garrytan/gstack@62091639, v1.12.2.0)
   ↓ (Phase 1~3 패턴 이식)
claude.gstack/   ← 표준 변형 (사용자 prompt 모드)
   ↓ (자율 진행 정책 추가)
claude.gstack.auto/  ← 이 변형
```

## claude.gstack 와의 차이점 (claude.gstack.auto 고유 산출물)

| 파일 | 변경 |
|---|---|
| `.claude/settings.json` | `permissions.allow` 광범위 패턴(`Bash(*)` 등) + `permissions.deny` 파괴 방지 + 신규 훅 wiring |
| `.claude/hooks/pre-bash-auto-boundary-check.sh` | 신규 — 인증/계정/외부경로 차단 |
| `.claude/agents/gatekeeper.md` | 신규 — 자율 모드 경계 결정 에이전트 |
| `CLAUDE.md` | "🤖 Autonomous Mode" 섹션 + 에이전트 표에 Gatekeeper 추가 + 디렉토리 트리 갱신 |

위 4개 파일을 제외하면 `claude.gstack` 와 동일한 산출물을 그대로 상속한다.

## 자율 진행 정책 (3 규칙 요약)

1. **작업 디렉토리 내부는 사용자 prompt 없이 자율 진행**
2. **모호한 액션은 에이전트끼리 (Gatekeeper → Reviewer/Architect) 검토 후 결정**
3. **계정 생성·인증·외부 디렉토리 부수 효과는 사용자 명시 승인 필수**

상세는 `CLAUDE.md` 의 "🤖 Autonomous Mode" 섹션 참조.

## 사용 권장 시나리오

- 신뢰할 수 있는 단독 개발자 환경 (퍼블릭 PR 워크플로우 전)
- 빠른 프로토타이핑·MVP 단계
- 에이전트 자율성을 최대화하고 사용자 인터럽트를 최소화하고 싶을 때

## 사용 비권장 시나리오

- 프로덕션 코드베이스 직접 작업
- 다중 개발자 동시 작업
- 보안 민감 프로젝트 (의료·금융·인증 시스템)
- 외부 API 결제·주문이 빈번한 프로젝트
