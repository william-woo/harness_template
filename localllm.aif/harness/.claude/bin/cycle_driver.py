...
def _aif_findings(role: str, files: list[str]) -> list[str]:
    """
    항목형 rubric 앙상블 판정을 실행해 judge 입력에 넣을 findings 를 만든다 (AIF — ADR-020).

    판정은 모델이 항목별로 하고(met/unmet/na + 증거), **집계는 결정론**이다. 확신이 없는 항목은
    지적으로 내보내지 않고 "판정 갈림" 으로만 표시한다 — 근거 없는 지적을 judge 에 주입하면
    거짓 revision 을 유발한다 (측정 08 의 거짓 부재 결함 계열).

    반복 2회가 기본이라 두 판정이 일치할 때만 지적이 된다. `CYCLE_AIF=0` 으로 끌 수 있고,
    어떤 실패에도 빈 목록을 반환해 사이클을 막지 않는다.
    """
    if os.environ.get("CYCLE_AIF", "1") == "0":
        return []
    ...
    try:
        ...
        return []
    rubric = "code-review" if role == "reviewer" else "qa-acceptance"
    ...
    _log(f"  ⓘ AIF 항목 판정({role}, {repeats}회): {agg['overall']} — 지적 {len(out_lines)}건")
    return out_lines
