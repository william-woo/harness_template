...
| **보안 (BLOCK)** | `.env`, `.env.*`, `*.pem`, `*.key`, `credentials.json`, `.aws/credentials`, `.netrc`, `id_rsa`, `id_dsa`, `id_ecdsa`, `id_ed25519`, `*.p12`, `*.pfx`, `*.jks` | **자격증명 노출 방지 — 누락 시 즉시 차단 (결정 5 예외). 단, `.example/.template/.sample` 접미사는 화이트리스트 (양식 파일 — BLOCK 면제)** |
