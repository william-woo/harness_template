...
| 2 | `LINT-STALE` | stale `in-progress` | 60일 이상 | 30~60일 | <30일 또는 checkpoint 0개 |
...
| 4 | `LINT-LEARN` | learnings 모순 | 없음 | 동일 topic에서 부정/긍정 키워드 동시 등장 (단순 휴리스틱) | confidence ≤ 5/10 또는 단일 entry type |
...
> **LINT-STALE 임계값 근거**: 세션 2 구현 단계에서 30일을 INFO 임계값으로 완화 — handoff 빈도 데이터 기반.

> **LINT-LEARN BLOCK 미구현 근거**: tombstone 기반 BLOCK은 향후 enhancement로 미루고, 초기 버전은 키워드 휴리스틱 CONCERN 수준으로 운영 — false positive 허용 ADR-004 결정 2 정신 일관.
