...
> auto+design+wiki+orch+hermes+pm+loop 전 스택을 담고 있습니다.
> 작업 디렉토리 내부 액션은 prompt 없이 자율 진행.
...
### 항목형 rubric 판정 (Phase 17 — F028, aif 변형 전용)

> **사용 가능 변형**: `claude.aif` / `localllm.aif` 만 (ADR-020 결정 1). 전체 적용은 부모와의
> 차이를 구별할 수 없게 만들므로 전용 변형에 격리한다. RLAIF/CAI 문헌의 판정 설계 규율 이식.

```
python3 .claude/bin/aif_judge.py plan code-review --target src/x.py --repeats 3  # 판정 프롬프트
python3 .claude/bin/aif_judge.py record code-review --run F001-r1 --file j1.txt  # 검증·저장
python3 .claude/bin/aif_judge.py aggregate F001-r1                              # 앙상블 집계
python3 .claude/bin/aif_judge.py compare code-review --a before.py --b after.py  # 순서 교대
python3 .claude/bin/aif_judge.py auto code-review --target src/x.py --run r1     # 로컬 LLM 전용
```

judge 에게 종합 판단이 아니라 **항목별 met/unmet/na + 증거**를 요구한다. `met` 인데 `파일:행`
이나 인용이 없으면 그 항목은 무효(UNCERTAIN)이고, N회 판정의 표가 갈리면 **억지로 정하지 않고
에스컬레이션**한다. 산문형 rubric 은 사람이 읽는 기준으로 그대로 남는다 (교체 아님).

**호출 기준**: judge 판정의 재현성이 낮을 때 / 판정자가 약한 모델일 때 / 통과 여부가 중요한
게이트일 때. 판정 호출이 N배로 늘어나므로 그 외에는 산문형 rubric + verify-loop 로 충분하다.
...
**aif(판정 설계) 오버레이** (claude.aif + localllm.aif 에만 — F028 신설):
- `.claude/bin/aif_judge.py` (항목형 rubric 판정 — 증거 검증 + 앙상블 + UNCERTAIN, stdlib)
