...
- 외부 의존성은 부모 상속분만. **aif 오버레이(aif_judge.py / items rubric / 커맨드)는 stdlib/문서뿐 — 신규 의존성 0**
- **판정은 모델이, 집계는 결정론이**: `aif_judge.py` 는 모델을 호출하지 않는다 (`auto` 모드만 예외 — cycle_driver 보유 변형에서만)
- 증거 없는 `met` 은 판정으로 인정하지 않고, 갈린 표는 `pass|revision` 으로 강제 기록하지 않는다 (UNCERTAIN → 에스컬레이션)
- **검증 범위**: `localllm.aif` 만 무인 스위트로 부모와 A/B 수치 비교가 가능하다. `claude.aif` 는 적합도 함수가 없어 **구조 이식까지** (ADR-019 결정 5 와 같은 비대칭)
- LINT-MR-14 가 aif 오버레이의 aif 계열 2 변형 전용 격리를 강제
