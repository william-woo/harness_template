---
description: "PM 주도 통합 제품 라이프사이클 (claude.productmgr 전용)"
---

# /product-cycle — PM 주도 통합 제품 라이프사이클 (claude.productmgr 전용)
...
```
/product-cycle "<제품 아이디어 / 요구사항>"          # 전체 (기획부터)
/product-cycle "<요청>" --from=design              # 중간 진입 (설계부터)
/product-cycle --feature=F0XX --from=develop       # 기존 feature 를 개발부터
/product-cycle --feature=F0XX --from=verify        # 구현된 코드를 검증부터
/product-cycle "<요청>" --from=design --to=develop # 구간 실행 (설계~개발만)
```
...
- 실행 라우팅만(제품 brief 불필요) → `/orchestrate`
- 설계 체인만 → `/plan-full`
