---
description: "디자인 토큰 선택·적용"
---

# /design-pick — 디자인 토큰 선택·적용
...
```
/design-pick                              # = compare (인자 없음 → 비교표)
/design-pick compare                      # 4 브랜드 비교표 (정적, LLM 없음)
/design-pick compare --format=json        # JSON 출력
/design-pick recommend                    # designer 에이전트 호출 안내
/design-pick apply apple                  # .claude/design/tokens.json 생성
/design-pick apply claude --force         # 기존 tokens.json 백업 후 덮어쓰기
/design-pick show                         # 현재 tokens.json 표시
/design-pick show --format=json           # JSON raw 출력
/design-pick self                         # 의존성·정합 점검
/design-pick self --strict                # BLOCK 있으면 exit 1
```
...
2. LLM 추천 원할 때
   /design-pick recommend
   → 안내에 따라 designer 에이전트 호출

3. 브랜드 결정 후 토큰 적용
   /design-pick apply apple
...
다음 중 하나라도 해당되면 `/design-pick` 실행을 권장한다:

- 새 UI 프로젝트 시작 시 디자인 정체성을 결정해야 할 때
- tokens.json 없는 상태에서 `/design-review` 실행 전
