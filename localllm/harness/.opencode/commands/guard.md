---
description: "최대 안전 모드"
---

# /guard — 최대 안전 모드

`/freeze` (편집 경계) + 기본 탑재된 `pre-bash-check.sh` (파괴 명령 차단)를 **모두 활성화**한 상태로 만든다.

현재 하네스에서 파괴 명령 차단(`rm -rf`, `DROP TABLE`, `git push --force` 등)은 항상 켜져 있으므로, `/guard`의 핵심 기능은 **편집 경계 설정**이다. 이름이 따로 있는 이유는 "프로덕션 건드릴 때 명시적으로 최대 보호 모드를 켠다"는 의도를 표현하기 위함.
...
받은 경로로 `/freeze`와 동일한 절차 실행 후 요약 출력:
