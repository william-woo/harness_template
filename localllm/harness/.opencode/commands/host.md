...
```
/host                         # 현재 호스트 정보 표시 (info)
/host set claude-code         # claude-code 로 설정
/host set openclaw            # openclaw 로 설정 (stub 안내 표시)
/host set codex               # codex 로 설정 (stub 안내 표시)
/host check                   # 무결성 점검 (무회귀 확인)
```
...
> 비-claude-code 호스트(codex, openclaw)로 `/host set` 또는 `render-skills`를
...
| `edit` | 파일 쓰기 | `Write` |
| `edit` | 파일 편집 | `Edit` |
| `edit` | 다중 편집 | `Edit` (MultiEdit 통합 — ADR-016) |
| `glob` | 파일 검색 | `Glob` |
| `grep` | 텍스트 검색 | `Grep` |
| `{{HOST.CMD.<name>}}` | 커맨드 호출 | `/project:<name>` |
| `$PWD` | 프로젝트 루트 환경변수 | `$CLAUDE_PROJECT_DIR` |
| `opencode` | 호스트 이름 | `claude-code` |
