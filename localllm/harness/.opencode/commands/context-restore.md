---
description: "저장된 컨텍스트 복원"
---

# /context-restore — 저장된 컨텍스트 복원

`/context-save`로 저장한 체크포인트 중 가장 최근 것(기본: 모든 브랜치 중)을 읽어, 이전 세션을 그대로 이어 받을 수 있도록 요약을 제시한다.
...
- `/context-restore`                     → 가장 최근 체크포인트 로드
- `/context-restore <제목-조각-또는-번호>` → 특정 체크포인트 로드
- `/context-restore list`                → "`/context-save list` 를 사용하세요" 안내 후 종료
...
```
ℹ️  저장된 컨텍스트가 없습니다.
    `/context-save` 로 현재 상태를 먼저 저장하면
    다음 세션에서 `/context-restore` 로 이어받을 수 있습니다.
