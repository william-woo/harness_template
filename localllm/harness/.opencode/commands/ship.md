...
```
/ship                     # 기본: 현재 브랜치 vs 기본 브랜치(main/master)
/ship <base-branch>       # 명시: 지정 브랜치와 비교
/ship --staged            # 스테이징된 변경만
```
...
필요한 리뷰:
  ...
  [ ] 2. 디자인 리뷰 (UI 변경 있음)
       → /design-review (정적 — 먼저 실행)
       → (통과 후) /qa-browser --feature=<Feature ID> (동적 E2E)
...
선택적:
  ...
  ? qa-browser — /qa-browser (Playwright 설치 후 또는 미설치 시 템플릿만)
════════════════════════════════════════
```
...
if [ "$UNCOMMITTED" -gt 0 ]; then
  echo "🚫 미커밋 변경 $UNCOMMITTED개 — /handoff 먼저"
fi
...
- **handoff.md와 중복 방지**: `/handoff`는 세션 종료(wip 가능), `/ship`은 
  "배포 가능 상태인가" 검사. 둘은 다른 시점에 호출.
- **Reviewer 에이전트와 관계**: `/ship`은 무엇을 리뷰해야 하는지 제안만.
