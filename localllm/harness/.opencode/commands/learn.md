...
- `/learn`              → **show** (최근 20개)
- `/learn search <질의>` → **search**
- `/learn add`          → **add** (대화형 추가)
- `/learn prune`        → **prune** (중복·상충 정리)
- `/learn stats`        → **stats**
- `/learn export`       → **export** (CLAUDE.md 섹션 형식으로)
...
tombstone = {
  ...
  "type": "tombstone",
  "key": "$KEY",
  "insight": "removed via /learn prune",
  "source": "user-stated"
}
...
`/start-session`은 자동으로 `/learn stats`를 실행해서 지식 양을 표시한다. 선택한 Feature와 유사한 key를 가진 학습이 있으면 planner 에이전트가 조회·참고한다 (`.claude/agents/planner.md` 참고).
