---
description: "자동 기획 파이프라인 (Autoplan)"
---

# /plan-full — 자동 기획 파이프라인 (Autoplan)
...
```
/plan-full <요구사항 또는 기능 설명>
```

예:
```
/plan-full 팀 할일 관리 앱 - 회원가입/로그인, 태스크 CRUD, 공유 기능
/plan-full F002 Autoplan 커맨드를 구현하고 싶다
/plan-full 결제 시스템을 추가하고 싶다 (토스페이먼츠 연동)
```
...
Architect는:
1. 관련 learnings 조회 (`/learn search <feature 키워드>`)
...
이런 경우는 기존 `/start-session` → Developer 경로를 유지.
